...
// (POR LO GENERAL SIEMPRE RECOMIENDO DEFINIR LAS CLASES CONSTRUCTORAS ARRIBA DE TODO O EN UN SCRIPT APARTE)
class Personas {
...
// (SIEMPRE ES MEJOR DECLARAR LAS VARIABLES AL PRINCIPIO DEL CÓDIGO CUANDO SON GLOBALES Y SE RE UTILIZAN EN EL CÓDIGO)
...
cantidadPersonas.onchange = () => {
  swal("", `¡Bien! Ahora completá los nombres de los morosos y cuanto gastó cada uno.`, "success");
};
  ...
  // LE PUSE DOS PARÁMETROS, EL PRIMERO DEBERIA SER UN NUMERO Y EL SEGUNDO EL NOMBRE
  const inputNombres = (n, nombre) => {

    $("#parcialesNombres").append('<input type="text" class="form-control parcialNombre rounded-pill" id='+"nombre-"+n+' placeholder="Nombre" aria-label="First name">')
 readonly="readonly"
    console.log("NOMBRE " + $("#nombreForm").val());
    console.log("ID : " + "#nombre-"+n);

    $("#nombre-"+n).val(nombre);
    
    }
    
  // LE PUSE DOS PARÁMETROS, EL PRIMERO DEBERIA SER UN NUMERO Y EL SEGUNDO EL GASTO DE CADA PERSONA
      const inputGastos = (n, gastos) => {
      $("#parcialesGastos").append('<input type="text" class="form-control parcialGasto rounded-pill" id='+"gastos-"+n+' placeholder="Gastó" readonly="readonly" aria-label="First name">')
      
      console.log("GASTOS: " + $("#gastosForm").val());
      console.log("ID : " + "#gastos-"+n);
...
entradaTotales.setAttribute("class", "inputGastosTotales form-control rounded-pill");
entradaTotales.setAttribute("step", "0.01");
// console.log(entradaTotales)
...
$("#botonTotalGastos").click(function(){
  ...
  for (const prop of storage){
    ...
    // Se muestra cada vez el valor por cada objeto
    console.log(Number(prop.gastos))
...
gastosPersona = $("#mostrarGastosPorPersona").val(gastosTotal/cantidadFinal)
...
if ($("#mostrarGastosPorPersona").val() < 1000){
    swal("Que barata salió la juntada che!! Cada uno tiene que pagar $" + $("#mostrarGastosPorPersona").val() )
} else{
    swal("Apa!! Que rompimos?? Cada uno tiene que pagar $" + $("#mostrarGastosPorPersona").val())
}
})
...
let totales_nombre = (n, nombre) => {
  $("#nombres").append('<input type="text" class="form-control" id='+"nombre-"+n+' placeholder="Nombre" aria-label="First name">')
  $("#nombre-"+n).val(nombre)
}
...
let nombresFinales = () => {
  ...
  for (i = 1; i <= cantidadFinal; i++){
      if (parseFloat($("#gastos-"+i).val()) < parseFloat($("#mostrarGastosPorPersona").val())) {
      aPagarCadaUno = parseFloat($("#mostrarGastosPorPersona").val()) - parseFloat($("#gastos-"+i).val())

      console.log($("#nombre-"+i).val() + " tiene que pagar $" + aPagarCadaUno)
    ...
    } else if (parseFloat($("#gastos-"+i).val()) > parseFloat($("#mostrarGastosPorPersona").val())){
      console.log($("#nombre-"+i).val() + " pagó de más $" +  (parseFloat($("#gastos-"+i).val()) - parseFloat($("#mostrarGastosPorPersona").val())))
      
      $("#nombre--"+i).val($("#nombre-"+i).val())
      $("#nombre2--"+i).val("Cobra :)")
      $("#gastos--"+i).val(parseFloat($("#gastos-"+i).val()) - parseFloat($("#mostrarGastosPorPersona").val()))
...
let cantidadFinal = Number(storage2.length)
  ...
  for (i = 1; i<=cantidadFinal; i++) {
$("#nombres").append('<input type="text" class="form-control rounded-pill" id='+"nombre--"+i+'  aria-label="First name">')[i]
$("#nombres2").append('<input type="text" class="form-control rounded-pill" id='+"nombre2--"+i+' aria-label="">')[i]
$("#gastos2").append('<input type="number" class="form-control rounded-pill" id='+"gastos--"+i+' step="0.01" aria-label="Gastos">')[i]
  }
  nombresFinales()
})
